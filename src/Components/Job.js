...
export default function Job({job}) {
  ...
  return (
    <div className="my-1 px-1 w-full  md:w-1/2 lg:my-4 lg:px-4 lg:w-1/3 p-2 ">
      ...
      <article className="overflow-hidden rounded-lg shadow-lg h-56 w-full bg-white">
        ...
        <div className="flex justify-start flex-col mt-6 space-y-3 pl-2">
          <div className="flex flex-row text-lg items-center space-x-8 justify-start ml-6 text-gray-600">
            <div><span> {formatDate (new Date(job.created_at))} </span> </div>
            <div> <span>{job.type} </span> </div>
          </div>
          <div>
            <h2 class="text-lg pr-4 font-bold leading-6 font-brand text-very-dark-blue dark:text-white ml-6">{job.title}</h2>
          </div>
          <div>
            <span className="ml-6 text-sm text-gray-600 ">{job.company}</span>
          </div> 
          <div>
            <span className="ml-6  text-indigo-600 font-semibold">{job.location}</span>
          </div>
        </div>
      </article>
